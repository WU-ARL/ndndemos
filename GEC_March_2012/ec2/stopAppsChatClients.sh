...
#EAST REGION
echo "WASHU: Stopping Chat clients homed off gateway:"
for h in $WASHU_HOSTNAMES 
do
  echo "$SSH_CMD_east_1 -o \"StrictHostKeyChecking no\" ubuntu@$h \"killall startApp.sh; killall ccnchat\""
  $SSH_CMD_east_1 -o "StrictHostKeyChecking no" ubuntu@$h "killall startApp.sh; killall ccnchat; killall java" 
done

echo "MEMPHIS: Stopping Chat clients homed off gateway:"
...
echo "UIUC: Stopping Chat clients homed off gateway:"
...
#WEST-2 REGION
echo "CSU: Stopping Chat clients homed off gateway:"
for h in $CSU_HOSTNAMES 
do
  echo "$SSH_CMD_west_2 -o \"StrictHostKeyChecking no\" ubuntu@$h \"killall startApp.sh; killall ccnchat\""
  $SSH_CMD_west_2 -o "StrictHostKeyChecking no" ubuntu@$h "killall startApp.sh; killall ccnchat; killall java" 
done

echo "ARIZONA: Stopping Chat clients homed off gateway:"
...
echo "PARC: Stopping Chat clients homed off gateway:"
...
#WEST-1 REGION
echo "CAIDA: Stopping Chat clients homed off gateway:"
for h in $CAIDA_HOSTNAMES 
do
  echo "$SSH_CMD_west_1 -o \"StrictHostKeyChecking no\" ubuntu@$h \"killall startApp.sh; killall ccnchat\""
  $SSH_CMD_west_1 -o "StrictHostKeyChecking no" ubuntu@$h "killall startApp.sh; killall ccnchat; killall java"
done

echo "UCI: Stopping Chat clients homed off gateway:"
...
echo "UCLA: Stopping Chat clients homed off gateway:"
...
echo "REMAP: Stopping Chat clients homed off gateway:"
